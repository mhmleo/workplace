...
```{r}
rca_panel |>
  distinct(period, post_nafta, exch_rate, fdiinflow, total_agri_fdi) |>
...
model4 <- feols(log(RCA) ~ post_nafta + log(exch_rate) + asinh(total_agri_fdi)|cmdCode,
               data = rca_panel,
               cluster = ~cmdCode)
...
### Monte Carlo Simulation Pseudo Code

```{r}
set.seed(123)
placebo_results <- replicate(3000, {
  fake_year <- sample(1992:2000, 1)
  rca_panel$fake_nafta <- if_else(rca_panel$period >= fake_year, 1, 0)
  coef(feols(RCA ~ fake_nafta + log(exch_rate) + log(fdiinflow) | cmdCode,
             data = rca_panel))["fake_nafta"]
})

# Plot distribution of placebo coefficients
hist(placebo_results, main = "Placebo Distribution")
abline(v = 0.451646, col = "red")
```
